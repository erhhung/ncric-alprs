...
class ALPRIntegration(Integration):
    ...
    def clean_row(cls, row, datasource=None):
        ...
        newdict['recordedby3_id'] = "_".join(
            filter(None, [str(row.red_id), str(row.standardized_agency_name), datasource]))
        ...
        newdict['collectedby2_id'] = "_".join(
            filter(None, [str(row.sourceid), str(row.standardized_agency_name), datasource]))
